...
function applyCameraWindow() {
  activeFrameWindow = frameFormat === 'png' && frameWindowSource ? transformedFrameWindow(frameWindowSource) : null;
...
function componentScore(component, width, height) {
  ...
  const edgeTouches = Number(component.minX === 0) + Number(component.minY === 0) + Number(component.maxX === width - 1) + Number(component.maxY === height - 1);
...
function detectTransparentWindow(image) {
  ...
  for (let start = 0; start < total; start += 1) {
    ...
    queue[tail++] = start;
    visited[start] = 1;
    let minX = width, minY = height, maxX = 0, maxY = 0, count = 0, containsCenter = false;
    while (head < tail) {
      ...
      if (x > 0 && !visited[left] && isTransparent(left)) { visited[left] = 1; queue[tail++] = left; }
      if (x + 1 < width && !visited[right] && isTransparent(right)) { visited[right] = 1; queue[tail++] = right; }
      if (y > 0 && !visited[up] && isTransparent(up)) { visited[up] = 1; queue[tail++] = up; }
      if (y + 1 < height && !visited[down] && isTransparent(down)) { visited[down] = 1; queue[tail++] = down; }
    }
  ...
  if ((boxW * boxH) / total < 0.035 || best.count / total < 0.02) return null;
  ...
  const x1 = clamp(best.maxX - insetX, x0, width - 1);
  const y1 = clamp(best.maxY - insetY, y0, height - 1);
...
function analyzeCurrentPngWindow() {
  ...
  try {
    ...
    applyCameraWindow();
    setStatus(frameWindowSource ? 'PNGの透過領域を自動認識し、撮影範囲を合わせました' : '大きな透過領域を検出できないため、全面を撮影範囲にします');
  } catch (err) {
...
function currentDigitalZoom() { return nativeZoomAvailable ? 1 : Math.max(1, zoomValue); }
  ...
  if (track && typeof track.getCapabilities === 'function') {
    try {
      ...
      if (zoom && Number.isFinite(zoom.min) && Number.isFinite(zoom.max) && zoom.max > zoom.min) {
        ...
        nativeZoomRange = { min: zoom.min, max: zoom.max, step: Number.isFinite(zoom.step) && zoom.step > 0 ? zoom.step : 0.1 };
...
function drawVideoIntoRect(ctx, video, dx, dy, dw, dh, mode = 'cover') {
  ...
  ctx.drawImage(video, sx, sy, cropW, cropH, dx + (dw - outW) / 2, dy + (dh - outH) / 2, outW, outH);
}
...
drawVideoCover = function drawVideoCoverWithWindowAndZoom(ctx, video, dw, dh) {
  if (frameFormat === 'png' && activeFrameWindow) {
    const b = activeFrameWindow;
    drawVideoIntoRect(ctx, video, Math.round(dw * b.x), Math.round(dh * b.y), Math.max(1, Math.round(dw * b.w)), Math.max(1, Math.round(dh * b.h)), 'cover');
    return;
  }
