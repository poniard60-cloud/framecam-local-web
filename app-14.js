...
function protectArtworkDistanceV8(data, width, height) {
  ...
  for (let i = 0; i < total; i += 1) {
    ...
    // Colored or dark pixels are considered foreground artwork.
    if (chroma >= ARTWORK_CHROMA_MIN_V8 || mean <= ARTWORK_DARK_MEAN_MAX_V8) {
      dist[i] = 0;
    }
  }

  // Two-pass Chebyshev distance transform. This protects white borders,
  // white eyes/text and anti-aliased edges surrounding colored artwork.
...
clearCheckerInsideWindowV7 = function clearCheckerInsideWindowPreserveWhiteV8(canvas, bounds) {
  ...
  // Scale the protection radius with the actual frame resolution.
  // This is large enough to keep the mascot's white outline, eyes and '&'.
  ...
  for (let i = 0; i < w * h; i += 1) {
    ...
    // Important: pure/near white is not globally transparent. It is only
    // removed when it belongs to the detected checker window AND is not near
    // protected artwork. This prevents white logos/outlines from disappearing.
...
// Rebuild the active frame immediately with the safer white-preservation rule.
