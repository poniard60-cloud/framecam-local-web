...
function smoothPinchRenderV3() {
  ...
  if (!nativeZoomAvailable) {
    // Digital zoom can be rendered directly at display refresh rate.
    els.video.style.transform = target > 1.001 ? `scale(${target})` : 'none';
    return;
  }

  // For native zoom, smoothly preview zoom-in with CSS. Zoom-out needs a
  // wider source image, so native constraints are updated at a low cadence
  // rather than shrinking the video and revealing empty borders.
...
function smoothPinchBeginV3(event) {
  ...
  // Prevent the older app-7 pinch listeners from also running.
...
function smoothPinchMoveV3(event) {
  ...
  // Slight damping makes small finger tremors less visible while preserving
  // the expected direct pinch feel.
  ...
  // Ignore sub-pixel-like changes that otherwise cause label/preview shimmer.
...
async function smoothPinchFinishV3(event) {
  ...
  if (nativeZoomAvailable) {
    // Keep the CSS preview in place until the camera reaches the final zoom.
    ...
    // If a throttled native update was already in flight, wait briefly for the
    // queued final value to drain without blocking the UI thread.
