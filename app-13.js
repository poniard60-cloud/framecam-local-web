...
applyCameraWindow = function applyCameraWindowUnderFrameV7() {
  ...
  activeFrameWindow = frameFormat === 'png' && finderWindowBoundsV7 ? { ...finderWindowBoundsV7 } : null;
...
analyzeCurrentPngWindow = function analyzeCurrentPngWindowUnderFrameV7() {
  ...
  try {
    ...
    if (typeof detectRealAlphaWindowV2 === 'function') alphaBounds = detectRealAlphaWindowV2(analysis);
...
drawAdaptiveOverlay = function drawAdaptiveOverlayUnderFrameV7(ctx, image, dw, dh) {
  if (frameFormat === 'png' && image === frameImage) {
    const matches = canonicalFrameCanvasV7 && canonicalFrameImageV7 === frameImage && canonicalFrameOrientationV7 === settings.orientation;
